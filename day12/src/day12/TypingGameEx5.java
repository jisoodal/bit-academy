...
{ 
      ...
      final Level[] LEVEL = {  
            new Level(500, 2000, 1000, data[0]),  // 1000넘으면 레벨업
            new Level(250, 1500, 2000, data[1]), // 2000넘으면 레벨업
            new Level(120, 1000, 3000, data[2]), // 3000넘으면 레벨업
            new Level(100, 500, 4000, data[3]), // 4000넘으면 레벨업
      }; 
      ...
      public Level getLevel(int level) { 
            if(level > MAX_LEVEL) level = MAX_LEVEL;  // 최고 레벨이면 최고 레벨 반환
            if(level < 0) level = 0; // 레벨 0이면 0 반환

            return LEVEL[level];  // 해당 레벨의 LEVEL 배열의 값(스피드, interval 등 넘겨줌-level로)
      } 
...
//                   되는지의 여부를 판단해서 true 또는 false를 반환한다.
    	  			...
    	  			// 모범답안: return score >= lev.levelUpScore; 문장 자체가 맞으면 true 아니면 false
      } 
...
//    	  		VirusThread.currentThread().interrupt(); -> 이렇게 하면 AWT eventThread가 interrupt됨!!
    	  			virusGrp.interrupt(); //  이것 때문에 레벨이 화면에 안띄워진 거임!!
...
//                      curLevel의 Level인스턴스를 얻어온다.(getLevel()사용)
    	  			...
    	  			// 모범 답안: Level lev = getLevel(curLevel++);
...
//                           1. 10번에 한번 꼴로 바이러스를 생성한다.(Math.random()사용)
 ...
 //                           boolean isVirus = (이곳에 알맞은 수식을 넣으세요.);
                        ...
                        // 1/10~9/10 -> 0.xxx이므로 int값 0과 같다. 따라서 1~9초까지는 isVirus가 false이고,(0이니까)
                        // 10/10 -> 1이므로 !=0의 문장과 부합한다. 따라서 10초에 한번 true를 반환한다.
